...
class Schema extends validator_1.Validator {
    ...
    makeSocketSchema() {
        return __awaiter(this, void 0, void 0, function* () {
            let socketJson = yield this.fetchSocketJSON();
            if (!socketJson) {
                return;
            }
            for (const k of Object.keys(socketJson)) {
                if (k === '$source') {
                    try {
                        ...
                        const extraYaml = js_yaml_1.default.safeLoad(fs_1.readFileSync(fn).toString());
                        socketJson = lodash_merge_1.default(socketJson, extraYaml);
                    }
                    catch (e) {
                        // Just pass
                    }
